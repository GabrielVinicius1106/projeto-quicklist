const listItens = document.querySelector('.items');
const newItemInput = document.querySelector('#new-item');
const addItemButton = document.querySelector('#add-item');

const REGEX = /\d+/g
const SVG_NS = "http://www.w3.org/2000/svg";

addItemButton.addEventListener("click", () => {
    
    let inputValue = newItemInput.value
    newItemInput.value = ""

    // Verifica se o input está vazio
    if (inputValue == "" || inputValue == null) {
        alert("O item não pode ser vazio!")
        return 0;
    }
    if (REGEX.test(inputValue) == true) {
        alert("O item não pode conter números!")
        return 0;
    }

    // Criar a div de item
    const newItem = document.createElement("div")
    newItem.classList.add("item");

    // Criar o checkbox do item
    const newItemContent = document.createElement("div")
    newItemContent.classList.add("checkbox");

    // Criar o input do checkbox
    const inputCheckbox = document.createElement("input");
    inputCheckbox.type = "checkbox";

    // Criar o label do checkbox
    const labelCheckbox = document.createElement("label");

    // Criar o span do checkbox
    const spanCheckbox = document.createElement("span");
    spanCheckbox.classList.add("check");

    // Criar o texto do item
    const itemText = document.createElement("span");
    itemText.innerText = inputValue;

    labelCheckbox.appendChild(spanCheckbox);
    labelCheckbox.appendChild(itemText);
    
    newItemContent.appendChild(inputCheckbox);
    newItemContent.appendChild(labelCheckbox);

    newItem.appendChild(newItemContent);
    
    listItens.appendChild(newItem);

    // Criar o ícone de remover
    const svg = document.createElementNS(SVG_NS, "svg");
    svg.classList.add("remove-item");
    svg.setAttribute("width", "20");
    svg.setAttribute("height", "20");
    svg.setAttribute("viewBox", "0 0 20 20");
    svg.setAttribute("fill", "none");
    svg.setAttribute("xmlns", "http://www.w3.org/2000/svg");

    const path1 = document.createElementNS(SVG_NS, "path");
    path1.setAttribute("d", "M16.25 4.58325L15.7336 12.9375C15.6016 15.0719 15.5357 16.1392 15.0007 16.9065C14.7361 17.2858 14.3956 17.606 14.0006 17.8466C13.2017 18.3333 12.1325 18.3333 9.99392 18.3333C7.8526 18.3333 6.78192 18.3333 5.98254 17.8457C5.58733 17.6047 5.24667 17.2839 4.98223 16.9039C4.4474 16.1354 4.38287 15.0667 4.25384 12.9293L3.75 4.58325");
    path1.setAttribute("stroke", "#6B6671");
    path1.setAttribute("stroke-width", "1.25");
    path1.setAttribute("stroke-linecap", "round");

    const path2 = document.createElementNS(SVG_NS, "path");
    path2.setAttribute("d", "M2.5 4.58342H17.5M13.3797 4.58342L12.8109 3.40986C12.433 2.6303 12.244 2.24051 11.9181 1.99742C11.8458 1.9435 11.7693 1.89553 11.6892 1.854C11.3283 1.66675 10.8951 1.66675 10.0287 1.66675C9.14067 1.66675 8.69667 1.66675 8.32973 1.86185C8.24842 1.90509 8.17082 1.955 8.09774 2.01106C7.76803 2.264 7.58386 2.66804 7.21551 3.47613L6.71077 4.58342");
    path2.setAttribute("stroke", "#6B6671");
    path2.setAttribute("stroke-width", "1.25");
    path2.setAttribute("stroke-linecap", "round");
    
    const path3 = document.createElementNS(SVG_NS, "path");
    path3.setAttribute("d", "M7.91687 13.75V8.75");
    path3.setAttribute("stroke", "#6B6671");
    path3.setAttribute("stroke-width", "1.25");
    path3.setAttribute("stroke-linecap", "round");

    const path4 = document.createElementNS(SVG_NS, "path");
    path4.setAttribute("d", "M12.0831 13.75V8.75");
    path4.setAttribute("stroke", "#6B6671");
    path4.setAttribute("stroke-width", "1.25");
    path4.setAttribute("stroke-linecap", "round");

    svg.appendChild(path1);
    svg.appendChild(path2);     
    svg.appendChild(path3);
    svg.appendChild(path4);
    newItem.appendChild(svg);
})